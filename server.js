...
Return ONLY the raw JSON with no extra text or formatting.`;
        ...
        console.log("Raw response from Gemini (Trip Plan):", text);
        
        const cleanedJsonText = extractJsonFromText(text);
        console.log("Cleaned JSON text (Trip Plan):", cleanedJsonText);
...
// API endpoint to generate blog post
app.post('/generate-blog', async (req, res) => {
    try {
        const { topic, language } = req.body;
        
        const prompt = `Write a blog about "${topic}" in ${language}.  
RETURN ONLY JSON in this format:
{
    ...
    "content": "Blog content here."
        ...
        const result = await model.generateContent(prompt);
        const text = await result.response.text();
        console.log("Raw response from Gemini (Blog):", text);
        
        const cleanedJsonText = extractJsonFromText(text);
        console.log("Cleaned JSON text (Blog):", cleanedJsonText);
        
        const jsonResponse = JSON.parse(cleanedJsonText);
        res.json(jsonResponse);
    } catch (error) {
        console.error('Error generating blog:', error);
        res.status(500).json({ error: 'Failed to generate blog', details: error.message });
    }
});
